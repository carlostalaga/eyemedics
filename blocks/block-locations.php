...
$locations_tabs_background = get_sub_field('tabs_background');
$locations_tabs_background_class = 'bg-white';

if ($locations_tabs_background === 'bg-hueso'):
    $locations_tabs_background_class = 'bg-hueso';
elseif ($locations_tabs_background === 'bg-white'):
    $locations_tabs_background_class = 'bg-white';
endif;

$locations_posts_query = new WP_Query(
    array(
...
?>
<div id="locations-<?php echo esc_attr($iBlock); ?>" class="container tabs-block py-0 px-0 <?php echo esc_attr($locations_tabs_background_class); ?>">
    <div class="container tabsBox py-0">

        <?php if ($locations_posts_query->have_posts()): ?>
        <?php
            $locations_posts = $locations_posts_query->posts;
            $locations_tab_index = 0;
        ?>

        <!-- Nav tabs -->
        <ul class="nav tabs-icon-nav justify-content-center flex-wrap" id="locationsTab-<?php echo esc_attr($iBlock); ?>" role="tablist">
            <?php foreach ($locations_posts as $locations_post): ?>
            <?php $locations_is_active = ($locations_tab_index === 0); ?>
            <li class="nav-item" role="presentation">
                <button class="tabs-icon-btn <?php echo $locations_is_active ? 'active' : ''; ?>" id="locations-tab-<?php echo esc_attr($iBlock); ?>-<?php echo esc_attr($locations_tab_index); ?>-tab" data-bs-toggle="tab" data-bs-target="#locations-tab-<?php echo esc_attr($iBlock); ?>-<?php echo esc_attr($locations_tab_index); ?>" type="button" role="tab" aria-controls="locations-tab-<?php echo esc_attr($iBlock); ?>-<?php echo esc_attr($locations_tab_index); ?>" aria-selected="<?php echo $locations_is_active ? 'true' : 'false'; ?>">
        ...
        <!-- Tab panes -->
        <div class="tab-content p-0" id="locationsTabContent-<?php echo esc_attr($iBlock); ?>">
            ...
            ?>
            <div class="tab-pane fade <?php echo $locations_is_active ? 'show active' : ''; ?>" id="locations-tab-<?php echo esc_attr($iBlock); ?>-<?php echo esc_attr($locations_tab_index); ?>" role="tabpanel" aria-labelledby="locations-tab-<?php echo esc_attr($iBlock); ?>-<?php echo esc_attr($locations_tab_index); ?>-tab">
                                ...
                                <div class="row g-3 align-items-center h-100">
                                    ...
                                    <div class="col-5">
                                        <a href="<?php echo esc_url($locations_doctor_profile_url); ?>" aria-label="View profile for <?php echo esc_attr($locations_doctor_name); ?>">
                                            <img src="<?php echo esc_url($locations_doctor_image_url); ?>" alt="<?php echo esc_attr($locations_doctor_name); ?>" class="img-fluid w-100 img-rounded-max">
                                        </a>
                                    </div>
                                    <?php endif; ?>
